...
if __name__ == "__main__":
    ...
    if(int(sys.argv[2])==0):
	    ...
	    contribs = links.join(ranks).flatMap(lambda url_urls_rank: computeContribs(url_urls_rank[1][0], url_urls_rank[1][1]))
		 ...
		 ranks = contribs.reduceByKey(add).mapValues(lambda rank : (rank *temp/100.0) + (100-temp)/100.0)
	    ...
	    if(sys.argv[3]=='0'):
		 ...
		 ranks = contribs.reduceByKey(add).mapValues(lambda rank : (rank *temp/100) + (100-temp)/100.0)
