...
if __name__ == "__main__":
	...
	initial_ranks = lines.map(lambda urls: initialiseRanks(urls)).distinct().groupByKey().mapValues(sum)
	ranks=initial_ranks.map(lambda x:(x[0],max(x[1],1))).cache()
