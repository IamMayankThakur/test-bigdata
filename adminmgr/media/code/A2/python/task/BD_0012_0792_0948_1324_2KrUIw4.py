...
def converge(o_rank,n_rank):
	...
	#while (i == 0): 
	if (abs(rank_sort1[0][1]-rank_sort2[0][1]) < 0.0001):
		cnt+=1
			#has_converged &= 1
		#i=1
		return 1
...
# Initialize the spark context.
	...
	ranks = ranks1.map(lambda x:(x[0],max(x[1],1))).cache()
	...
	if(int(sys.argv[2]) == 0):
		...
		while (i == 0 ):
			...
			ranks = contributs.reduceByKey(add).mapValues(lambda rank:( rank * weight + (1-weight)) if weight else rank * 0.8 + 0.2)
			n_rank=ranks
