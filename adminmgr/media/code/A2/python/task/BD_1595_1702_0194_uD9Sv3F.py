...
def parNeighfunc2(y):
	chunks = re.split(r',', y)
	x = float(chunks[2])/float(chunks[3])
...
if(int(sys.argv[2]) == 0):
	...
	while(not(i)):
		...
		out2 = sorted(rank(),key = sortfunc())
		valcheck = abs(out1[0][1] - out2[0][1])
		ranks = temprank
		
		if(valcheck > pow(10,-4)):
			i =  False
	...
	ranks = sorted(rank(),key= sortfunc())
