...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print('Usage: pagerank <number_iterations> <rank_weights>',
              file=sys.stderr)
        sys.exit(-1)
    ...
    links = lines.map(lambda batsman: parseNeighbors(batsman,
                      0)).distinct().groupByKey()
    ranks = lines.map(lambda b: parseNeighbors(b,
                      1)).distinct().groupByKey().map(lambda x: (x[0],
            (sum(x[1]) if sum(x[1]) > 1.0 else 1.0)))
    ...
    for (link, rank) in ranks.sortBy(lambda a: (-a[1], a[0])).collect():
