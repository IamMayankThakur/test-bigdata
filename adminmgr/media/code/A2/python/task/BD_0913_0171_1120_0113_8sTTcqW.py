...
if __name__ == "__main__":
	...
	links = l.map(lambda urls: parseNeighbors(urls)).distinct().groupByKey()
	#print(links)
	...
	if (int(sys.argv[2])) == 0:		#For 0 iterations
		...
		while flag == 1 and i < 105:
			...
			flag=0
			difference = previous_ranks.leftOuterJoin(ranks).map(lambda r: (r[0],abs(r[1][0]-r[1][1])) )
