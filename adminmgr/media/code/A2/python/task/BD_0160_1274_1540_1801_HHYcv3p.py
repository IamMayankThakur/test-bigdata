...
if __name__ == "__main__":
    ...
    bowlers = links.map(lambda record:(record[0],record[1])).groupByKey().cache()
    sumx = links.map(lambda record:(record[0],record[2])).reduceByKey(add).cache()
    ...
    ranks=ranks.sortByKey().sortBy(lambda x: -x[1])
    for (link, rank) in ranks.collect():
