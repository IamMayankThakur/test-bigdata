...
if __name__ == "__main__":
    ...
    #lines = spark.read.text("file:///home/raunak/a2/BowlerRankTestData.txt").rdd.map(lambda r : r[0]).map(lambda urls : re.split(',+', urls))
    ...
    while(notconv and ((iter_count < n_iters) if n_iters > 0 else True)):
        old_ranks = ranks
        contribs = links.join(old_ranks).flatMap(lambda person_opps_rank : computeContribs(person_opps_rank[1][0], person_opps_rank[1][1]))
