...
for line in sys.stdin:
    line = line.strip()
    line_val = line.split("\t")
    key, val, ball_val = line_val[0], line_val[1], line_val[2]
    try:
        wicket_count = int(val)
        ball_count = int(ball_val)
    except ValueError:
        continue
    if current_key == key:
        current_wicket_count += wicket_count
        current_ball_count += ball_count
    else:
        if (current_key != "" and current_ball_count > 5):
            value = (current_wicket_count,current_ball_count,current_key)
