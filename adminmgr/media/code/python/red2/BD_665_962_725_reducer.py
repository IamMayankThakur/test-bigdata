...
for line in sys.stdin:
	line = line.strip()
	line_val = line.split(",")
	bowler,batsman,run,deli = line_val[0], line_val[1], line_val[2], line_val[3]
	try:
		count_run  = int(run)
		count_deli = int(deli)
	...
	if(current_bowler == bowler and current_batsman == batsman):
		...
		if (current_batsman != ""):
			if (current_deli > 5): 
				...
				Dict[current_bowler,current_batsman] = current_run,current_deli
		current_run = count_run
		current_deli = count_deli
...
if(current_bowler == bowler and current_batsman == batsman):
	if(current_deli > 5): 
		#print('%s\t%s\t%d\t%d'% (current_batsman,current_bowler,current_wic,current_deli))
		Dict[current_batsman,current_bowler] = current_run,current_deli
