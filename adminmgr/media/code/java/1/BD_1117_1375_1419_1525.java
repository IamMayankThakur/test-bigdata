...
import java.util.*;
public class BD_1117_1375_1419_1525{
    ...
    public static void main(String[] args) throws Exception {
        ...
        job.setJarByClass(BD_1117_1375_1419_1525.class);
...
class CMapper extends Mapper<LongWritable, Text, Text, Text> {
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        if(Data.length>=9)
        {
            if(Data[9].length()>=3 && !Data[9].equals("run out") && !Data[9].equals("retired hurt"))
            {
                context.write(new Text(Data[4]+","+Data[6]), new Text("1,1"));
            }
            else
            {
                context.write(new Text(Data[4]+","+Data[6]), new Text("0,1"));
            }
        }
    }
}
...
class CReducer extends Reducer<Text, Text, Text, IntArrayWritable> {
                ...
                while(i.hasNext())
                {
                    ...
                    if(lol1 == (Integer)wicket)
                    {
                        ...
                        if(lol2 == (Integer)delivery)
                        {
                            l = c.listIterator(j);
                            lol3 = (String)l.next();
                            if(wd[0].compareTo(lol3)==0)
                            {
                                m = d.listIterator(j);
                                lol4 = (String)m.next();
                                
                                if(wd[1].compareTo(lol4)<0)
                                    break;
                            }
                            else if(wd[0].compareTo(lol3)>0)
                                break;
                        }
    ...
    protected void cleanup(Context context) throws IOException, InterruptedException
    {
        ...
        while(i.hasNext() && j.hasNext() && k.hasNext() && l.hasNext())
        {
            ...
            context.write(new Text(((String)k.next())+","+((String)l.next())),new IntArrayWritable(temp));
        }
    }
