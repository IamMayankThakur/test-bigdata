...
public class BD_005_105_295{
	...
	public static class iplMapper extends Mapper<LongWritable, Text , Text, Text>{
		...
		String regex="\\s+$";
     		...
     		if(valueStr[0].charAt(0) =='b')
		{
                    	String keyUserID = valueStr[6]+","+valueStr[4];
			String keyUserID1=keyUserID.replaceAll(regex,"");
			
			int result = Integer.parseInt(valueStr[7]) + Integer.parseInt(valueStr[8]);
			String result_1 = Integer.toString(result);
			context.write(new Text(keyUserID1), new Text(result_1));
	...
	public static class iplCombiner extends Reducer<Text,Text, Text, Text> {
		...
		if(ballCount>5)
		{	
			String op= Integer.toString(runCount) + "," + Integer.toString(ballCount);
