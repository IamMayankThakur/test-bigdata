...
public class BD_152_554_125 {
    ...
    extends Mapper<Object, Text, Text, Text>{
		...
		String val0 = "0";
        	...
        	{
			if(Record[10].equals(Record[4]) && !(Record[9].equals("run out")) && !(Record[9].equals("retired hurt")))
			{				
				String batsman = Record[4];
				String bowler = Record[6];
				context.write(new Text(batsman + "," + bowler), new Text(val1 + "," + val1));
			}
			else
			{
				String batsman = Record[4];
				String bowler = Record[6];
				context.write(new Text(batsman + "," + bowler), new Text(val0 + "," + val1));
			}
		}
        }
    }
    ...
    extends Reducer<Text,Text,Text,Text> {
	    ...
	    int balls = 0;
            ...
            {
		wickets += Integer.parseInt(value.toString().split(",")[0]);
            	balls += Integer.parseInt(value.toString().split(",")[1]);
            }
	    if(balls > 5)
	    {
		BD_152_554_125.results.put(key.toString(), wickets + "," + balls);
		context.write(new Text(""), new Text(wickets + "," + balls));
	    }
        }
    }
