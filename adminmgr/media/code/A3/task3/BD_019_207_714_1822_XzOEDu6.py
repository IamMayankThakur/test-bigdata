...
if __name__=="__main__":
	...
	windowedWordCounts = job.reduceByKeyAndWindow(lambda x, y: x + y, lambda x, y: x - y, a, b)
#	windowedWordCounts = job.countByValueAndWindow(a,b)
	def gunf(time,rdd):		
		val=sorted(rdd.collect(),key=lambda x:(-x[1],x[0]))
		i=0
		if(len(val)>4):
			print("%s,%s,%s,%s,%s"%(val[0][0],val[1][0],val[2][0],val[3][0],val[4][0]))
