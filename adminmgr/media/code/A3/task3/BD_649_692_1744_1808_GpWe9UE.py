...
def process_rdd(rdd):  
    sort = rdd.sortBy(lambda x: (-x[1],x[0])).filter(lambda y: y[0] !='')
    sort_list=sort.collect()
    if(sort_list!=[]):
    #print(sorted_list[:5])
        print(sort_list[0][0],sort_list[1][0],sort_list[2][0],sort_list[3][0],sort_list[4][0],sep=",")
