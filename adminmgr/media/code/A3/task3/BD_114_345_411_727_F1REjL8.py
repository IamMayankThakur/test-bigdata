...
dd = dataStream.window(int(sys.argv[1]), int(sys.argv[2]))

hashTags=dd.flatMap(lambda x: splitHashtags(x.split(';')[7].split(','))).\
        filter(lambda y: len(y[0])>=1)
        
hashTagsCount=hashTags.\
        reduceByKeyAndWindow(lambda agg,obj:agg+obj, lambda agg, obj : agg - obj, int(sys.argv[1]), int(sys.argv[2]))
'''
...
commonHashTags=hashTagsCount.\
        transform(lambda rdd: rdd.sortBy(-x[1], x[0]))\
'''
commonHashTags = hashTagsCount.transform(lambda rdd: rdd.sortBy(lambda x: (-x[1], x[0])))        
