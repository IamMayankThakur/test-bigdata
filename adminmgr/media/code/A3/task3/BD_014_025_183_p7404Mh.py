...
def echo(time, rdd):
	counts = rdd.collect()
	if(len(counts)!=0):
		print(counts[0][0],",",counts[1][0],",",counts[2][0], sep="")
def tmp(x):
	splitter = x.split(';')
...
dataStream = ssc.socketTextStream("localhost",9009).window(int(sys.argv[1]),int(sys.argv[2]))
newStream = dataStream.flatMap(tmp)
finalStream = newStream.reduceByKey(add)
sortedStream = finalStream.transform((lambda foo:foo.sortBy(lambda x:(-x[1]))))
top_three_hash = sortedStream.transform(lambda rdd:rdd.context.parallelize(rdd.take(3)))
