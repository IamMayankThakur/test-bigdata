...
def process_rdd(time, rdd):
		...
		try:
			...
			hashtag_counts_df = sql_context.sql("select tweetid, no_of_tweets from hashtags limit 5")
			'''
			hashcount  = hashtag_counts_df.rdd.map(lambda x:x.tweetid).collect()
			print(','.join(hashcount))
			'''
			i=0
			for row in hashtag_counts_df.rdd.collect():
				i+=1
				if(i==5):
					print(row.tweetid)
					i=0
				else:
					print(row.tweetid,end=',')
...
if __name__ == "__main__":
	if(len(sys.argv)!=3):
		print("Who will enter window size???")
		sys.exit(-1)
	wind = int(sys.argv[1])
	...
	#count_values_windowed.pprint(3)
