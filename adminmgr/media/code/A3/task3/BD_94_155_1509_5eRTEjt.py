...
# dataStream.pprint()
tweet=dataStream.window(float(sys.argv[1]),1).map(tmp)
...
#sort_t=count.transform(lambda rdd: rdd.sortByKey(lambda x:x[0]).sortBy(lambda x:(x[1],x[0]), ascending=False))
sort_t=count.transform(lambda rdd: rdd.sortBy(lambda x:(-x[1],x[0])))
p_print(sort_t)
#print(type(sort_t))
