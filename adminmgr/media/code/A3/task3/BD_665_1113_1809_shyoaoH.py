...
if __name__ == "__main__":
	if len(sys.argv) != 3:
		sys.exit(-1)
	if int(sys.argv[2])<0 or int(sys.argv[1])<0 :
		sys.exit(-1)			
	conf=SparkConf()
	conf.setAppName("BigData")
	sc=SparkContext(conf=conf)
	ssc=StreamingContext(sc,int(sys.argv[1]))#according to be batch interval is this
	#ssc.checkpoint("/home/chaitra/Desktop/assignment3/checkpoint_BIGDATA")
	data=ssc.socketTextStream("localhost",9009)
	dataStream=data.window(int(sys.argv[2]), 1)#window,sliding
