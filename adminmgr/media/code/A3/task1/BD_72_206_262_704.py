...
df2 = df.select(posexplode(split("hashtags",",")).alias("pos","val"))
df3 = df2.groupBy("val").count().sort(col("count").desc()).limit(3)
df3.writeStream.outputMode("complete").format("console").start().awaitTermination(20)