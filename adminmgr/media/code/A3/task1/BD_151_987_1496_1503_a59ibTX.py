...
inputDF = spark.readStream.schema(schema).option("delimiter",";").option("delimiter",";").option("delimiter",";").option("maxFilesPerTrigger",1).csv(inputPath)
...
query1=query1.writeStream.outputMode("complete").format("console").option("numRows",5).start()
