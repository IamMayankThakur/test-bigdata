...
my_schema = StructType().add('ID','integer')\
.add('Lang','string')\
.add('Date','string')\
.add('Source','string')\
.add('Len','integer')\
...
Hashtag_counts_dataframe=spark_session_object.sql("select Name as name,MAX(Followers/Friends) as FRRatio from table group by Name order by FRRatio desc limit 5")
