...
tweets=dataStream.map(out).filter(lambda x:x[0]!="Other").reduceByKeyAndWindow(lambda x, y: x + y, int(sys.argv[1]), 20) #reduce the input based on the window size
#tweets = tweets.transform(lambda rdd : rdd.ctx.parallelize(rdd.top(5)))
tweets = tweets.transform(lambda rdd : rdd.ctx.parallelize(rdd.top(3,key=lambda x: x[1]))) #select top 3 hashtags
