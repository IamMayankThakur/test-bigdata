...
#"ID","language","Date","source","len","likes","RTs","Hashtags","Usernames","Userid","name","Place","followers","friends"
    ...
    lines = spark\
        ...
        .option("sep", ";")\
        .schema(user_schema)\
        .csv("/home/chaitra/Desktop/stream")

    # Split the lines into words
    words = lines.select(
        # explode turns each item in an array into a separate row
        lines.name, lines.followers/lines.friends
    )

    # Generate running word count
    wordCounts = words.groupBy(lines.name).agg({"(followers / friends)": "sum" })
    wordCounts = wordCounts.select(col("name"), col("sum((followers / friends))").alias("FRRatio"))	
    wordCounts = wordCounts.sort(desc("FRRatio")).limit(1)
